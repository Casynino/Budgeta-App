...
import { mockBudgets, mockDebts, mockGoals, mockInvestments, mockRecurringPayments } from '../data/mockData';
import { accountsAPI, preferencesAPI, transactionsAPI } from '../services/api';
...
export const FinanceProvider = ({ children }) => {
   ...
   */
  const [baseCurrency, setBaseCurrency] = useState('TZS'); // Currency for storage (all amounts stored in this)
  const [displayCurrency, setDisplayCurrency] = useState('TZS'); // Currency for display
      ...
      try {
        ...
        if (!user) {
          ...
          if (savedCurrency) {
            ...
            setBaseCurrency(currencyData.base || 'TZS');
            setDisplayCurrency(currencyData.display || 'TZS');
          } else {
            // If nothing saved yet, default both to TZS for offline users
            setBaseCurrency('TZS');
